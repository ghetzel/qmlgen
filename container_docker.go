...
func (self *DockerContainer) Tail() <-chan *LogLine {
...
func (self *DockerContainer) portmap() map[nat.Port][]nat.PortBinding {
	...
	if len(self.ConfigPorts) > 0 {
		for outer, inner := range self.ConfigPorts {
			var p, t = stringutil.SplitPair(outer, `/`)

			if p == `` {
				log.Warningf("bad port spec %q: %v", outer, inner)
				continue
			}

			if t == `` {
				t = DefaultContainerPortTransport
			}

			out[nat.Port(typeutil.NInt(inner))] = []nat.PortBinding{
				{
					HostPort: fmt.Sprintf("%s/%s", p, t),
				},
			}
		}
	}
